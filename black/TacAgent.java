...
public class TacAgent extends Agent {
    ...
    protected void setup() {
        System.out.println("Tac-agent "+getAID().getName()+" (WHITE) is ready.");
        ...
        tacGui.showGUI();
        tacGui.updateBoardVisual(othello.board);
        ...
        public void action() {
            if (step == 0) {
                ...
                if (msg != null) {
                    ...
                    reply.addReceiver(new AID("tic", AID.ISLOCALNAME));
                    send(reply);
                    step = 1; // enter play stage
                    // White (Tac) initially not turn; Tic (black) will start
                }
            }
        }
    }
                ...
                if (msg != null && !msg.getContent().equals(lastMsg)) {
                    ...
                    if (content.equals("PASS")) {
                        ...
                        if (othello.hasAnyValidMove(0)) {
                            ...
                            m.setContent("PASS");
                            m.addReceiver(new AID("tic", AID.ISLOCALNAME));
                            send(m);
                        }
                    }
                }
    ...
    void updateBoard(String content){
        ...
        // send move to opponent (tic = black)
